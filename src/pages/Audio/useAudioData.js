...
export const AudioDataProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const audioSubscription = client.subscribe(`databases.${databaseId}.collections.${audioCollectionId}.documents`, (response) => {
            ...
            if (events.includes('databases.*.collections.*.documents.*.update')) {
                setAudioData(prevData => prevData.map(audio => audio.$id === payload.$id ? payload : audio));
            }

            if (events.includes('databases.*.collections.*.documents.*.delete')) {
                setAudioData(prevData => prevData.filter(audio => audio.$id !== payload.$id));
            }
        });
    ...
    const addAudio = async (newAudio) => {
        try {
            await db.createDocument(databaseId, audioCollectionId, ID.unique(), newAudio);
        } catch (err) {
            setError(err);
            throw err;
        }
    ...
    const updatePlayed = async (audioUrl) => {
        try {
            const audioToUpdate = audioData.find(audio => audio.audioUrl === audioUrl);
            if (!audioToUpdate) {
            ...
            await db.updateDocument(databaseId, audioCollectionId, audioToUpdate.$id, {
                played: updatedAudio.played
            });
            
            setAudioData(prevData => prevData.map(audio =>
    ...
    const updateAudio = async (audioId, updatedAudio) => {
        try {
            await db.updateDocument(databaseId, audioCollectionId, audioId, updatedAudio);
            setAudioData(prevData => prevData.map(audio =>
    ...
    const deleteAudio = async (audioUrl) => {
        try {
          const documentId = audioData.find(audio => audio.audioUrl === audioUrl).$id;
